...
class Character {
  constructor(name, type) {
    this.name = name
    this.type = type
    this.getInfo = () => `This is a ${this.type} character named ${this.name}.`
  }
}
...
class NPC extends Character {
  ...
  this.phrase = phrase
  this.dialogue = () => `${this.name}: ${this.phrase}`
  }
}
...
class Hero extends Player{
  constructor(name, type, healthLevel, attackLevel, superPowers = []){
    ...
    this.addSuperPower = (power) => this.superPowers.push(power)
    this.useSuperPower = (index) => `${this.name} used ${this.superPowers[index]}!`
  }
}
...
const spittingFire = fireSpitter.addSuperPower('spitting fire')
const flying = fireSpitter.addSuperPower('flying')
const spikedTail = fireSpitter.addSuperPower('spiked tail')
